...
{
	var ambientProduct = mult(lightAmbient, selectedShape[MATERIAL_AMBIENT]);
	var diffuseProduct = mult(lightDiffuse, selectedShape[MATERIAL_DIFFUSE]);
    var specularProduct = mult(lightSpecular, selectedShape[MATERIAL_SPECULAR]);
	
    gl.uniform4fv(gl.getUniformLocation(program, "ambientProduct"), flatten(ambientProduct));
    gl.uniform4fv(gl.getUniformLocation(program, "diffuseProduct"), flatten(diffuseProduct) );
    gl.uniform4fv(gl.getUniformLocation(program, "specularProduct"), flatten(specularProduct) );	
    gl.uniform4fv(gl.getUniformLocation(program, "lightPosition"), flatten(lightPosition) );
    gl.uniform1f(gl.getUniformLocation(program, "shininess"), selectedShape[MATERIAL_SHININESS]);
}
...
function onAddButtonClick(event)
{
	...
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(shape[SHAPE_NORMALS]), gl.STATIC_DRAW);
	normalBuffer.normalSize = 3;
	normalBuffer.numItems = shape[SHAPE_NORMALS] / 3;
...
function render() 
{
	...
	for (var i = 0; i < n; i++)
	{
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, normalBuffer);
        gl.vertexAttribPointer(program.vNormal, normalBuffer.normalSize, gl.FLOAT, false, 0, 0);
